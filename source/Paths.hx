...
class Paths
{
		...
		for (key in currentTrackedAssets.keys())
		{
			...
			if (!localTrackedAssets.contains(key) && !dumpExclusions.contains(key))
			{
				...
				if (obj != null)
				{
					...
					if (uniqueRAMImages.contains(key))
						uniqueRAMImages.remove(key);
					if (uniqueVRMImages.contains(key))
						uniqueVRMImages.remove(key);
				}
			}
		}
	...
	inline public static function animateAtlas(path:String, ?library:String):String
	{
	...
	inline static public function image(key:String, ?library:String, ?throwToGPU:Bool = true):FlxGraphic
	{
	...
	inline static public function getSparrowAtlas(key:String, ?library:String, ?throwToGPU:Bool = true):FlxAtlasFrames
	{
		#if MODS_ALLOWED
		...
		return FlxAtlasFrames.fromSparrow((imageLoaded != null ? imageLoaded : image(key, library, throwToGPU)),
			(xmlExists ? File.getContent(modsXml(key)) : file('images/$key.xml', library)));
		#else
		return FlxAtlasFrames.fromSparrow(image(key, library, throwToGPU), file('images/$key.xml', library));
		#end
	}
	...
	inline static public function getPackerAtlas(key:String, ?library:String, ?throwToGPU:Bool = true)
	{
		#if MODS_ALLOWED
		var imageLoaded:FlxGraphic = returnGraphic(key, library, throwToGPU);
		...
		return FlxAtlasFrames.fromSpriteSheetPacker((imageLoaded != null ? imageLoaded : image(key, library, throwToGPU)),
			(txtExists ? File.getContent(modsTxt(key)) : file('images/$key.txt', library)));
		#else
		return FlxAtlasFrames.fromSpriteSheetPacker(image(key, library, throwToGPU), file('images/$key.txt', library));
		#end
	}
	...
	{
		return formatToSongPathNoLowerCase(path).toLowerCase();
	}

	inline static public function formatToSongPathNoLowerCase(path:String)
	{
		var invalidChars = ~/[~&\\;:<>#]/;
		var hideChars = ~/[.,'"%?!]/;

		var path = invalidChars.split(path.replace(' ', '-')).join("-");
		return hideChars.split(path).join("");
	}

	// completely rewritten asset loading? fuck!
	public static var currentTrackedAssets:Map<String, FlxGraphic> = [];

	public static function returnGraphic(key:String, ?library:String, allowGPU:Bool = true, folder:String = 'images')
	{
		...
		#end
		{
			file = getPath('$folder/$key.png', IMAGE, library);
			if (uniqueRAMImages.contains(file))
				uniqueRAMImages.remove(file);
			if (uniqueVRMImages.contains(file))
				uniqueVRMImages.remove(file);
			if (currentTrackedAssets.exists(file))
			{
				localTrackedAssets.push(file);
				return currentTrackedAssets.get(file);
	...
	static public function cacheBitmap(file:String, ?bitmap:BitmapData = null, ?allowGPU:Bool = true)
	{
		...
		if (allowGPU && ClientPrefs.GPUCaching)
		{
			...
			if (!uniqueVRMImages.contains(file))
				uniqueVRMImages.push(file);
			uniqueRAMImages.remove(file);
		}
		else
		{
			if (!uniqueRAMImages.contains(file))
				uniqueRAMImages.push(file);
			uniqueVRMImages.remove(file);
		}
	...
	static function cneImage(key:String, ?library:String, checkForAtlas:Bool = false, ?ext:String = "png")
	{
		if (checkForAtlas)
		{
			var atlasPath = getPath('images/$key/spritemap.$ext', IMAGE, library);
			var multiplePath = getPath('images/$key/1.$ext', IMAGE, library);
		...
		return getPath('images/$key.$ext', IMAGE, library);
	}
	...
	public static function getFrames(key:String, assetsPath:Bool = false, ?library:String)
	{
		...
		return tempFramesCache[key] = loadFrames(assetsPath ? key : Paths.cneImage(key, library, true));
	}
}

private class MultiFramesCollection extends FlxFramesCollection
{
